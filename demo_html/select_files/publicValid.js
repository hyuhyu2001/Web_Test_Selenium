...
;(function(){
	...
	validTip.defaults={
		...
		right:function(obj,text){//正确提示
			text=text||'';
//			var current = $(obj).closest('p');
//			current.find('strong').remove('.font_red');
//			current.find('span').remove('.icon_wrong').remove('.icon_right');
//			current.find('.tips_box').prev().after("<span class='icon_right'></span>");
			$(obj).parent().find(".hint").html(text);
			if(text){
			   $(obj).parent().find(".hint").css('display','');
			}else{
			  $(obj).parent().find(".hint").css('display','none');
			}
		},
...
//			current.find('strong').remove('.font_red');
			current.find('span').remove('.icon_wrong').remove('.icon_right');
			current.find('.tips_box').prev().after("<span class='icon_wrong'></span>");*/
...
//			current.find('strong').remove('.font_red');
//			current.find('span').remove('.icon_wrong').remove('.icon_right');
//			current.find('.tips_box:not(:.allow_null)').prev().after("<strong class='font_red'>&nbsp;*</strong>");
			//current.find('.hint').html(text);
			//$(obj).val('');
			if(text){
				$(obj).parent().find(".hint").css('display','');
			}else{
		...
		clear:function(obj,text){//清除时
			text=text||'';
			//var current = $(obj).closest('p');
//			current.find('strong').remove('.font_red');
//			current.find('span').remove('.icon_wrong').remove('.icon_right');
			//current.find('.tips_box').prev().after("<strong class='font_red'>&nbsp;*</strong>");
			if(text){
				$(obj).parent().find("span").css('display','');
			}else{
				$(obj).parent().find("span").css('display','none');
			}
			//$(obj).closest('p').find('.password_info').removeClass('right').removeClass('wrong').html(text).css('display','inline-block');
		}
	};
	//合并默认配置
...
;(function(){
	...
	var d=document,
		...
		regEmail=/^[\w.-]+?@([0-9a-z]+\.[a-z]{2,6}(\.[a-z]{2})?)$/i,
		//身份证
		...
		regEmpty=/[^\s]{1,}/;
		//验证账号
		regAccount=/^[a-zA-Z]+[a-zA-Z0-9_]{5,19}$/;
	/*顶级私有函数*/
	...
	var valid=(function(){
		...
		valid.fn=valid.prototype={
			...
			userAccount:function(config){
				config.limit=config.limit||regAccount;
				return valid.fn.core(config);
			},
			//验证初始化
