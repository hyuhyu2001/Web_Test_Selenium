...
var s=["province_ul","city_ul"];//三个select的name
var opt0 = ["请选择","请选择"];//初始值
function _init_area(){  //初始化函数
	/*for(i=0;i<s.length-1;i++){
	  document.getElementById(s[i]).onchange=new Function("change("+(i+1)+")");
	}
	change(0);*/
	$.each(dsy.Items[0],function(i){
		$("#province_ul").append('<li ><a href="javascript:void(0);"lang='+[i]+' class = "province_li">'+dsy.Items[0][i]+'</a></li>');
	});
	//省市 --start
	 $(".province_li").unbind('click').bind('click',function(el){
	 	$("#province_selectDropDown").hide();
	    $("#province").val($(this).text());
	 	$("#city_ul li").remove();
	 	var cityList = dsy.Items['0_'+$(this).attr('lang')];
	 	$("#city_selectDropDown").show();
	 	$("#city").val("");
	 	$.each(cityList,function(i){
			$("#city_ul").append('<li ><a href="javascript:void(0);" class = "city_li">'+cityList[i]+'</a></li>');
		});
		//企业类型点击下拉列表取值
		$(".city_li").unbind('click').bind('click',function(el){
	...
	/*
		for(i=0;i<s.length-1;i++){
			var itemDefaultValue = optDefaultValue[i];
			var current = document.getElementById(s[i]);
			var value = $(current).val();
			var count = 0;
			var flag = true;
			$(current).find("option").each(function(){
				if(count!=0&&flag){
					if(itemDefaultValue==$(this).val()){
						//$(current).find("option").first().remove();
						$(current).find("option").removeAttr("selected");
	...
	*/
	/*var j = 0;
	if($("#province_value").val() != null || $("#city_value").val() != null ){
	var optDefaultValue = [$("#province_value").val(),$("#city_value").val()];
		$.each(s,function(){
			var item = $("#"+this);
			var itemDefaultValue = optDefaultValue[j];
			var current = document.getElementById(s[j]);
			var count = 0;
			var flag = true;
...
//单独专为注册使用的初始化函数
//解决验证信息的显示和隐藏
//create by xuan 2015-09-24
function _init_area_for_register(){  //初始化函数
	 $.each(dsy.Items[0],function(i){
		$("#province_ul").append('<li ><a href="javascript:void(0);"lang='+[i]+' class = "province_li">'+dsy.Items[0][i]+'</a></li>');
	 });
	 //省
	...
	//市
	$("#city_ul").on('click','.city_li',function(){
	   if($(this).text()!=""){
	   			$("#area").unbind('focus').attr("readonly",false).removeClass("noEditable");
		}else{
			$("#area").attr("readonly",true).addClass("noEditable").val("").focus(function(){
				$(this).blur();
			});
		}
	   $("#city_selectDropDown").hide();
	...
	if($('#flag').val()==1){
		//省市赋值
		var flagProvince = $.trim($('#flagProvince').val());
		var flagCity = $.trim($('#flagCity').val());
		//$('#province').val(flagProvince);
		//$('#city').val(flagCity);
		if(!bnu(flagProvince)){
			$("#province_ul li").each(function(i,n){
				if($.trim($(n).text())==flagProvince){
					$(this).find('.province_li').click();
					$("#city_ul li").each(function(i,n){
						if($.trim($(n).text())==flagCity){
							$(this).find('.city_li').click();
						}
					});
				}
